...
from app.models import Lot, Order
...
def _load_lot(db: Session, lot_id: int) -> Lot | None:
    return db.query(Lot).filter(Lot.id == lot_id).with_for_update().first()


def _remaining_special_fractions(lot: Lot) -> int:
    return max(0, lot.special_price_fractions_cap - (lot.sold_special_fractions or 0))
...
def settle_order_payment(
    ...
    *,
    ...
    """Mark an order as paid and increment lot sold fractions when validations pass."""
    try:
        ...
        lot = _load_lot(db, order.lot_id)
        if not lot:
            return _rollback_and_result(
                db,
                PaymentSettlementResult(
                    status=PaymentSettlementStatus.LOT_NOT_FOUND,
                    order_id=order.id,
                    lot_id=order.lot_id,
                ),
            )

        capacity_validation_result = _validate_lot_capacity(db, order=order, lot=lot)
